...
```
1, chema 标准化转换（create_agent 初始化阶段）
...
3. 第三步：Schema 真正交给模型（每次推理调用阶段）
Schema 真正被发送给大模型，只有一个时机：Agent 执行链走到模型调用节点、发起 API 请求的那一刻。
完整时序：

用户输入触发 Agent 执行
Agent 完成上下文组装、历史消息拼接、工具权限校验等前置逻辑
调用第二步生成的、绑定了 tools 的 Runnable 模型实例
Runnable 自动把提前绑定的 Schema 合并到本次请求的参数中
把 messages + tools schema + 其他参数 一起发给大模型 API
...
如果工具抛异常，它不会直接让整轮崩掉，而是返回一个错误 ToolMessage：

```python
ToolMessage(
    content="Tool call failed: Tool 'xxx' failed after ...",
